...
local Memes = {
    "amogus",
    "eminem actin real quiet after bambi dropped 'vs dave and bambi' on us",
    "part limit: exists, vsb players: BAD 😡😡😡😡😡"
}
            ...
            while true do
                ...
                if Update == true then
                    if Type == "Main" then
                        ...
                        Screen.CFrame =
                            Screen.CFrame:Lerp(
                            owner.Character.HumanoidRootPart.CFrame * CFrame.new(0, 2, -5) *
...
local consoleText2, console2 =
    CreateLib:CreateLockedScreen(
    CFrame.new(-7, 0, 1, 0.965925813, 0, 0.258819044, 0, 1, 0, -0.258819044, 0, 0.965925813),
    "Additional"
)
local consoleText3, console3 =
    CreateLib:CreateLockedScreen(
    CFrame.new(7, 0, 1, -0.965925813, 0, 0.258819163, 0, 1, 0, -0.258819163, 0, -0.965925813) *
...
local function warn(text, textbox)
    textbox.Text = textbox.Text .. warning_text:format(text) .. "\n"
end
...
local function print(color, textbox, text)
    ...
    textbox.Text = textbox.Text .. print_text:format(math.round(r), math.round(g), math.round(b), text) .. "\n"
end
                        ...
                        if i == "NLS" then
                            return function(s, p)
                                print(
                                    Color3.new(1, 1, 1),
                                    consoleText1,
                                    module .. " is requesting a localscript. Allow (req [yes/no])"
                                )
                                repeat
                                    task.wait()
                                until answered == true
    ...
    ["lang"] = function(args)
        ...
        if GetLang("CheckLanguage", l) == "true" then
            ...
            print(
                Color3.new(0, 0.5, 1),
                consoleText1,
                'lang: switched language to <font color="#00D1FF">' .. propify(Language) .. "</font>."
            )
        else
